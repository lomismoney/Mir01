...
1. **安裝依賴**
   ...
   > **注意**: 
   > - 路徑應該是相對於 `Mir01/inventory-api` 的相對路徑。如果您在專案根目錄，請使用 `cd inventory-client`。
   > - `npm install` 會自動執行 `postinstall` 腳本來生成 API 類型定義檔案 (`src/types/api.ts`)
   > - 如果需要手動重新生成 API 類型，可以執行 `npm run api:types`

...
## 2025-01-12 訂單收款後頁面同步問題

### 問題描述
點擊「確認收款」或記錄部分收款後，頁面需要手動刷新才能看到更新的訂單狀態。

### 原因分析
- React Query 的 `invalidateQueries` 使用 `refetchType: 'active'` 只會重新獲取正在被組件使用的查詢
- 某些情況下，訂單詳情頁面的查詢可能沒有被標記為 active

### 解決方案
增強快取同步機制，修改 `useConfirmOrderPayment` 和 `useAddOrderPayment` hooks：
...
### 關鍵改動
1. 將 `refetchType` 從 `'active'` 改為 `'all'`
2. 新增 `refetchQueries` 作為雙重保險機制
3. 使用 `Promise.all` 確保所有操作同時執行
4. 將 `onSuccess` 改為 async 函數

### 修改檔案
- `src/hooks/queries/useEntityQueries.ts` - `useConfirmOrderPayment` 和 `useAddOrderPayment` hooks

...
export function ChartAreaInteractive({ productVariantId }: ChartAreaInteractiveProps) {
  ...
  // 調用數據獲取 Hook
  const { data: chartData, isLoading, error } = useInventoryTimeSeries({
  ...
  const stats = useMemo(() => {
    ...
    const changePercent = startQuantity > 0 ? (change / startQuantity) * 100 : 0
    const quantities = chartData.map(d => d.quantity)
    const maxQuantity = Math.max(...quantities)
    const minQuantity = Math.min(...quantities)

    return { startQuantity, endQuantity, change, changePercent, maxQuantity, minQuantity }
  }, [chartData])
  ...
  return (
    <Card className="@container/card">
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        {/* 圖表渲染 */}
        {!isLoading && !error && productVariantId && chartData && chartData.length > 0 && (
          <div className="space-y-4">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <YAxis
                      ...
                      tickMargin={8}
                      tick={{ fontSize: 12 }}
                      domain={[Math.max(0, stats.minQuantity - 5), stats.maxQuantity + 5]}
                    />
                    <ChartTooltip
                      content={
                        <ChartTooltipContent 
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="space-y-1">
                ...
                <div className="text-sm font-medium">{stats.startQuantity.toLocaleString()}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground">期末庫存</div>
                <div className="text-sm font-medium">{stats.endQuantity.toLocaleString()}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground">變化量</div>
                <div className={`text-sm font-medium ${stats.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>

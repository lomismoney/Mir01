...
export function UserStoresDialog({
  ...
  // 獲取用戶當前的分店
  const { data: userStoresData, isLoading: isLoadingUserStores } = useUserStores(userId);
  ...
  // 獲取所有分店
  const stores: Store[] = Array.isArray(storesData?.data) ? storesData?.data as Store[] : [];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            為 <strong>{userName}</strong> 選擇所屬的分店。用戶可以管理這些分店的庫存和相關數據。
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button 
            ...
            disabled={assignStoresMutation.isPending || isLoadingStores || isLoadingUserStores}
          >

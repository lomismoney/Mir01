...
export function TransferStatusEditDialog({
  ...
  const getAvailableStatuses = (currentStatus: string) => {
    ...
    switch (currentStatus) {
      case 'pending':
        return allStatuses.filter(s => ['pending', 'in_transit', 'completed', 'cancelled'].includes(s.value))
      case 'in_transit':
        return allStatuses.filter(s => ['in_transit', 'completed', 'cancelled'].includes(s.value))
      case 'completed':
                ...
                <FormItem>
                  ...
                  {form.watch('status') !== transfer.status && (
                    <p className="text-sm text-muted-foreground">
                      {getStatusChangeDescription(transfer.status || '', form.watch('status'))}
                    </p>
                  )}
                  <FormMessage />
                </FormItem>

...
export default function PerformanceMonitor() {
    ...
    const collectMetrics = () => {
      ...
      try {
        ...
        if (navigation && navigation.loadEventEnd > 0) {
          ...
          pageLoadTime = document.readyState === 'complete' ? 
      ...
      const newMetrics: PerformanceMetric[] = [
        {
          ...
          unit: '秒',
          status: pageLoadTime < 2000 ? 'excellent' : pageLoadTime < 5000 ? 'good' : 'warning',
  ...
  const getIcon = (name: string) => {
    if (name.includes('頁面')) return <Clock className="w-4 h-4" />;
    if (name.includes('API')) return <Network className="w-4 h-4" />;
    if (name.includes('路由')) return <Zap className="w-4 h-4" />;
    if (name.includes('中間件')) return <Timer className="w-4 h-4" />;
    if (name.includes('快取')) return <Database className="w-4 h-4" />;

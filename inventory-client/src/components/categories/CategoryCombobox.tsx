 ...
 */
export function CategoryCombobox({ categories, value, onChange, disabled }: CategoryComboboxProps) {
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] max-h-[--radix-popover-content-available-height] p-0">
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              {/* 不指定分類選項 */}
              <CommandItem value="null" onSelect={() => { onChange(null); setOpen(false); }}>
              ...
              {categories.map((category) => (
                <CommandItem
                  ...
                  onSelect={() => {
                    if (!category.hasChildren) { // 只有在不是父分類時才可選
                  ...
                  className={cn({ 
                    "font-bold text-muted-foreground cursor-not-allowed": category.hasChildren,
                ...
                >
                  ...
                  />
                  <span className="truncate">{category.displayPath}</span>
                  {category.hasChildren && (
                    <span className="ml-auto text-xs text-muted-foreground">（含子分類）</span>
                  )}
                </CommandItem>
              ))}
            </CommandGroup>
